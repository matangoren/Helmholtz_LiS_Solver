...
function gpu_flexible_gmres(
    ...
    flexible::Bool=false,
    mem::FGMRESmem=getEmptyFGMRESmem()
)
    ...
    if storeInterm
        X = zeros(eltype(b), n, maxIter) |> gpu
    end
    ...
    if out >= 0
        ...
        if flag == -1
            println(
                @sprintf(
                    "fgmres iterated maxIter (=%d) outer iterations without achieving the desired tolerance. Acheived: %1.2e",
                    maxIter,
                    err
                )
            )
        ...
        elseif flag == 0 && out >= 1
            println(
                @sprintf(
                    "fgmres achieved desired tolerance at inner iteration %d. Residual norm is %1.2e.",
                    counter,
                    resvec[counter]
                )
            )
