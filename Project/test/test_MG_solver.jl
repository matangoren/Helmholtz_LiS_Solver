...
initial_n = [64, 64]
kappa_coeff = [[0.2], [0.9], [0.25,1.0], [0.5, 1.0], [0.75,1.0]]
...
for i=3:3
    ...
    for c in kappa_coeff
        ...
        omega = (0.2*pi) / (maximum(h)) # wkh = 0.2pi
        # omega = (0.2*pi) / (maximum(h)*maximum(kappa)) # wkh = 0.2pi
        gamma = getABL(n,false,ones(Int64,2)*12,omega) .+ 0.05*(omega/maximum(sqrt.(m)))#.+ 0.01*4*pi
